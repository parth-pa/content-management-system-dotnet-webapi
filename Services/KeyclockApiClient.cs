...
namespace keyclock_Authentication.Services
{
    public class KeyclockApiClient
    {
        ...
        public async Task<HttpResponseMessage> GetAccessToken(UserLogin users)
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, "http://10.195.81.4:8080/realms/CMS1/protocol/openid-connect/token");
        ...
        public Userinformation getUserData(string userId, string token)
        {
            ...
            HttpResponseMessage registerRequet = _httpClient.GetAsync($"http://10.195.81.4:8080/admin/realms/CMS1/users/{userId}").Result;
            ...
            var bodyContent = new FormUrlEncodedContent(new[]
        {
            ...
            var requestForToken = new HttpRequestMessage(HttpMethod.Post, "http://10.195.81.4:8080/realms/master/protocol/openid-connect/token");
