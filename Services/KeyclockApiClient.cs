...
namespace keyclock_Authentication.Services
{
    public class KeyclockApiClient
    {
        ...
        public async Task<HttpResponseMessage> GetAccessToken(UserLogin users)
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, "http://localhost:8080/realms/CMS1/protocol/openid-connect/token");
        ...
        public Userinformation getUserData(string userId, string token)
        {
            ...
            HttpResponseMessage registerRequet = _httpClient.GetAsync($"http://localhost:8080/admin/realms/CMS1/users/{userId}").Result;
